import { FilePenIcon, PencilIcon, PlusIcon, UploadCloudIcon, Trash2, XIcon, TrashIcon } from 'lucide-react'
...
const Dashboard = () => {
    ...
    const colors = ['#9333EA', '#D97706', '#dc2626',  '#2563eb', '#16a34a']
    ...
    return (
        <div> 
            <div className='max-w-7xl mx-auto px-4'>
                ...
                bg-clip-text text-transparent sm:hidden'>Welcome, Nikal Prajapati</p>

                <div className='flex gap-4'>
                    <button onClick={() => setShowCreateResumes(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center 
                    ...
                    <button onClick={()=> setShowUploadResumes(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center 
                    justify-center rounded-lg gap-2 text-slate-600 border border-dashed
                    ...
                    {allResumes.map((resume, index)=> {
                        ...
                        return (
                            <button key={index} onClick={()=> navigate(`/app/builder/${resume._id}`)}
                             className='relative w-full sm:max-w-40 h-32 flex flex-col items-center justify-center rounded-lg 
                             gap-3 border group hover:shadow-lg transition-all cursor-pointer'
                            ...
                            <p className='text-sm font-medium text-center' style={{ color:baseColor }}>{resume.title}</p>
                  ...
                  className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50
                    z-10 flex items-center justify-center">
                        ...
                        border shadow-md rounded-lg w-full max-w-sm p-6'>
                            ...
                            <input type='file' id='resume-input' accept='.pdf,.word,docs' hidden
                ...
                {editResumeId && (
                    ...
                    z-10 flex items-center justify-center">
                        ...
                        border shadow-md rounded-lg w-full max-w-sm p-6'>
                            ...
                            <button className='w-full py-2 bg-green-600 text-white rounded
                            hover:bg-green-700 transition-colors'>Edit Resume</button>
                            <XIcon className='absolute top-4 right-4 text-slate-400
                            hover:text-slate-600 cursor-pointer transition-colors' onClick={() => 
